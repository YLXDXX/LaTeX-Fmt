...
static void print_help(const char* prog) {
    std::cerr
        ...
        << "  --no-cjk-spacing / --cjk-spacing\n"
        << "                           Disable / enable CJK-ASCII auto spacing\n"
        << "  --no-brace-completion / --brace-completion\n"
        << "                           Disable / enable brace completion (\\frac12 → \\frac{1}{2})\n"
        << "  --no-comment-normalize / --comment-normalize\n"
        << "                           Disable / enable comment normalization (%comment → % comment)\n"
        << "  --no-blank-line-compress / --blank-line-compress\n"
        << "                           Disable / enable blank line compression\n"
        << "  --keep-trailing-spaces / --remove-trailing-spaces\n"
        << "                           Keep / remove trailing whitespace\n"
        << "  --no-display-math-format / --display-math-format\n"
        << "                           Disable / enable display math standalone line formatting\n"
        << "  --no-math-unify / --math-unify\n"
        << "                           Disable / enable math delimiter unification\n"
        << "  --inline-math-style=S    Inline math output style: dollar ($), bracket (\\(\\))\n"
        << "                           (default: dollar)\n"
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg.compare(0, 20, "--inline-math-style=") == 0) {
            std::string val(arg.substr(20));
            if (val == "dollar" || val == "$") {
                config.inline_math_style = latex_fmt::InlineMathStyle::Dollar;
            } else if (val == "bracket" || val == "\\(") {
                config.inline_math_style = latex_fmt::InlineMathStyle::Bracket;
            } else {
                std::cerr << "latex-fmt: error: invalid inline math style '" << val
                          << "'. Valid: dollar, bracket\n";
                return 1;
            }
            continue;
        }
