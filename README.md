...
- 正文中，中文和英文字符间需要加入空格，中文标点与英文字符间不需要加入空格
- 正文中，中文与行内公式间需要加入空格，中文标点与行内公式间不需要加入空格
- 除正文内容外，像在公式中，没有中英文间加空格的要求，原样输出即可
...
- 如果某行仅由空格和 tab组成，删除该行的全部空格和tab，变为一个空行

- 对于空行的处理，仅针对于代码中有连续多个空行的情况，将连续多个换行压缩成一个换行，对于单个空行保持原样

- 对于非单行注释（行尾注释），注释符号 `%` 与注释内容间保留一个空格

- 对于单行注释（注释单独成一行）， `%` 前面的空格数量与上下文的缩进相同，即注释也需要缩进，并且 `%` 后面与注释内容间也是留一个空格

- 对于行间公式 `$$公式内容$$` 不能接在正文内容一行的后面，后面同行也不需要接正文内容，前后如果有内容则需要添加换行，换行后变为
  ...
  这里的公式内容按照行间公式的内容进行格式化，同样要保持缩进，与 equation 的环境一样，只是前后换成了 $$ 而已

## 当前代码问题分析 

### 架构级问题 

1. **Lexer 不解析 `$...$` 数学模式**：`$` 被当作普通 Text，导致无法区分行内数学与文本，进而无法实现 CJK-数学间距、数学空格保留等功能 
2. **`$$...$$` 显示数学未特殊处理**：README 要求显示公式独立成行并缩进，但当前仅把 `\[...\]` 替换为 `$$` 而无格式化 
3. **无 CJK-英文间距逻辑**：`visit(Text&)` 只压缩空格，从未在 CJK 与 ASCII 之间主动添加空格 

### 格式化 Bug 

1. **纯空白行未转为空行**：`"   \t  "` 行被当作 Text 处理，不会产生空行效果 
2. **行尾空格未删除**：输出末尾可能有残留空格 
3. **注释规范化缺失**：`%comment` 不会变为 `% comment`，行末注释前的空格处理不完善 
4. **连续空行压缩不彻底**：仅靠 ParBreak 处理双换行，三行以上空白行中间的空格/文本干扰了检测 
5. **显示数学 `$$` 未独立成行**：应前后换行、内容缩进 
